...
import DslDataModelSpell from "../data-model-spell";

export const key = "dsl.item.datamodel.spell";
export const options = { displayName: "DSL: Item: Data Model: Spell" };

export default ({ describe, it, expect }: QuenchMethods) => {
  describe("manualTags()", () => {
    const spell = new DslDataModelSpell();
  ...
  describe("autoTags()", () => {
    ...
    it("Roll create autoTags", () => {
      ...
      expect(spell.autoTags[3].label).equal(
        `${game.i18n.localize("DSL.items.Roll")} 1d20+1`
      );
    });
  });
};
