...
import DslDataModelArmor from "../data-model-armor";

export const key = "dsl.item.datamodel.armor";
export const options = { displayName: "DSL: Item: Data Model: Armor" };

export default ({ describe, it, expect }: QuenchMethods) => {
  describe("ArmorTypes", () => {
    const dataTypes = DslDataModelArmor.ArmorTypes;

    it("Has 4 armor types", () => {
      expect(Object.keys(dataTypes).length).equal(4);
    });
    it("Has unarmored", () => {
      expect(dataTypes.unarmored).equal("DSL.armor.unarmored");
    });
    it("Has light", () => {
    ...
    it("Has shield", () => {
      expect(dataTypes.shield).equal("DSL.armor.shield");
    });
  });

  describe("manualTags", () => {
    const item = new DslDataModelArmor();
  ...
  describe("autoTags", () => {
    ...
    it("By default return light armor", () => {
      expect(item.autoTags[0].label).equal("DSL.armor.light");
    });
