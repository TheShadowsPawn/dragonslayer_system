...
const DslCombat = {
      ...
      if (game.user.isGM) {
        ...
        if (actorData.isSlow) {
          await data.combatants[i].update({
            initiative: DslCombat.STATUS_SLOW,
          });
        } else {
  ...
  async individualInitiative(combat, data) {
    ...
    for (const [i, roll] of rollArr.entries()) {
      ...
      let value = cbt.actor?.system?.isSlow
        ? DslCombat.STATUS_SLOW
        : roll.total;
      // check if actor is defeated
      if (combat.settings.skipDefeated && cbt.isDefeated) {
        value = DslCombat.STATUS_DIZZY;
      }
      updates[i].initiative = value;

      // render template
      const template = `${DSL.systemPath()}/templates/chat/roll-individual-initiative.html`;
      const tData = {
        name: cbt.name,
    ...
    evalRoll.content = `
    <details>
    <summary>${game.i18n.localize("DSL.roll.individualInitGroup")}</summary>
    ${msgContent}
    </details>`;
  ...
  format(object, html, user) {
    html.find(".initiative").each((_, span) => {
      span.innerHTML =
        span.innerHTML === `${DslCombat.STATUS_SLOW}`
          ? '<i class="fas fa-weight-hanging"></i>'
          : span.innerHTML;
      span.innerHTML =
        span.innerHTML === `${DslCombat.STATUS_DIZZY}`
      ...
      controls
        ...
        .after(
          `<a class='combatant-control move-combat ${moveActive}' title="${game.i18n.localize(
            "DSL.CombatFlag.RetreatFromMeleeDeclared"
      ...
      controls
        ...
        .after(
          `<a class='combatant-control prepare-spell ${spellActive}' title="${game.i18n.localize(
            "DSL.CombatFlag.SpellDeclared"
          )}"><i class='fas fa-magic'></i></a>`
        );
    });
    DslCombat.announceListener(html);
    ...
    html.find(".combatant").each((_, ct) => {
      ...
      controls.prepend(
        `<a class='combatant-control flag' style='color:${color}' title="${CONFIG.DSL.colors[color]}"><i class='fas fa-flag'></i></a>`
      );
    });
    DslCombat.addListeners(html);
  },
  ...
  addListeners(html) {
    ...
    html.find(".combatant-control.flag").click((ev) => {
      ...
      const colors = Object.keys(CONFIG.DSL.colors);
    ...
    html.find('.combat-control[data-control="reroll"]').click((ev) => {
      ...
      const data = {};
      DslCombat.rollInitiative(game.combat, data);
      if (game.user.isGM) {
  ...
  addCombatant(combat, data, options, id) {
    ...
    combat.updateSource({ flags: { dsl: { group: color } } });
  },
  ...
  addContextEntry(html, options) {
    options.unshift({
      ...
      callback: DslCombat.activateCombatant,
    });
  },
  ...
  async preUpdateCombat(combat, data, diff, id) {
    ...
    if (data.round !== 1) {
      if (reroll === "reset") {
        DslCombat.resetInitiative(combat, data, diff, id);
        return;
      }
      if (reroll === "keep") {
        return;
    ...
    if (init === "group") {
      DslCombat.rollInitiative(combat, data, diff, id);
    } else if (init === "individual") {
      DslCombat.individualInitiative(combat, data, diff, id);
    }
  },
};

export default DslCombat;
