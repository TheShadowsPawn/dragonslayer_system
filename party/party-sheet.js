...
import DslPartyXP from "./party-xp";

const Party = {
  partySheet: void 0,
};

export default class DslPartySheet extends FormApplication {
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["dsl", "dialog", "party-sheet"],
      template: `${DSL.systemPath()}/templates/apps/party-sheet.html`,
      width: 280,
      height: 400,
      resizable: true,
      dragDrop: [
  ...
  static init() {
    Party.partySheet = new DslPartySheet();
  }

  static showPartySheet(options = {}) {
    DslPartySheet.partySheet.render(true, { focus: true, ...options });
  }
   ...
   */
  get title() {
    return game.i18n.localize("DSL.dialog.partysheet");
  }
  ...
  getData() {
    ...
    return {
      partyActors: DslParty.currentParty,
      // data: this.object,
      config: CONFIG.DSL,
      user: game.user,
      settings,
  ...
  async _dealXP(ev) {
    new DslPartyXP(this.object, {}).render(true);
  }
