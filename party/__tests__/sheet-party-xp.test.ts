...
export const key = "dsl.party-xp.sheet";
export const options = { displayName: "DSL: Party XP: Sheet" };

export default ({ describe, it, expect, assert }: QuenchMethods) => {
  describe("defaultOptions()", () => {
    it("Has correctly set defaultOptions", () => {
      const partyXP = new DslPartyXP();
      expect(partyXP.options.classes).contain("dsl");
