 ...
 */
import DslActor from "./actor/entity";
...
function applyChatCardDamage(html: JQuery, multiplier: 1 | -1) {
  ...
  if (dmgTgt === CONFIG.DSL.apply_damage_options.originalTarget) {
  ...
  if (dmgTgt === CONFIG.DSL.apply_damage_options.targeted) {
    game.user?.targets.forEach((t) => applyDamageToTarget(t.actor, amount, multiplier, t.name));
  }
  if (dmgTgt === CONFIG.DSL.apply_damage_options.selected) {
...
async function applyDamageToTarget(actor: Actor | null, amount: string, multiplier: 1 | -1, nameOrId: string) {
  if (!game.user?.isGM || !(actor instanceof DslActor)) {
    ui.notifications?.error(game.i18n.format("DSL.error.cantDealDamageTo", { nameOrId }));
    return;
  }
...
function canApplyDamage(html: JQuery) {
  ...
  switch (applyDamageOption) {
    case CONFIG.DSL.apply_damage_options.originalTarget:
      return !!html.find(".chat-target").last().data("id");
    case CONFIG.DSL.apply_damage_options.targeted:
      return !!game.user?.targets?.size;
    case CONFIG.DSL.apply_damage_options.selected:
      return !!canvas.tokens?.controlled.length;
    default: {
      ui.notifications?.error(game.i18n.format("DSL.error.unexpectedSettings", {
...
export const addChatMessageContextOptions = (
  ...
  options.push(
    {
      name: game.i18n.localize("DSL.messages.applyDamage"),
      icon: '<i class="fas fa-user-minus"></i>',
      condition: canApply,
      callback: (li) => applyChatCardDamage(li, 1),
    },
    {
      name: game.i18n.localize("DSL.messages.applyHealing"),
