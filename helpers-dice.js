/**
 * @file Helpful methods for dealing with DSL-specific dice logic
 */
...
const DslDice = {
  async sendRoll({
    ...
    const template = `${DSL.systemPath()}/templates/chat/roll-result.html`;
    ...
    templateData.result = DslDice.digestResult(data, roll);

    return new Promise((resolve) => {
      roll.render().then((r) => {
        templateData.rollDSL = r;
    ...
    if (game.settings.get(game.system.id, "ascendingAC")) {
      ...
      if (this.attackIsSuccess(roll, targetAac, attackBonus)) {
        result.details = game.i18n.format(
          "DSL.messages.AttackAscendingSuccess",
          {
            result: roll.total,
          }
        );
        ...
        result.details = game.i18n.format(
          "DSL.messages.AttackAscendingFailure",
          {
            bonus: result.target,
          }
        );
      ...
      result.details = game.i18n.format("DSL.messages.AttackFailure", {
        bonus: result.target,
      });
      result.isFailure = true;
    }
  ...
  async sendAttackRoll({
    ...
    const template = `${DSL.systemPath()}/templates/chat/roll-attack.html`;
    const chatData = {
      user: game.user.id,
      speaker,
      flags,
    ...
    templateData.result = DslDice.digestAttackResult(data, roll);
  ...
  async RollSave({
    ...
    let rolled = false;
    const template = `${DSL.systemPath()}/templates/chat/roll-dialog.html`;
    const dialogData = {
      formula: parts.join(" "),
      data,
    ...
    if (skipDialog) {
      return DslDice.sendRoll(rollData);
    }

    const buttons = {
      ok: {
        label: game.i18n.localize("DSL.Roll"),
        icon: '<i class="fas fa-dice-d20"></i>',
        callback: (html) => {
          rolled = true;
          rollData.form = html[0].querySelector("form");
          roll = DslDice.sendRoll(rollData);
        },
      },
      magic: {
        label: game.i18n.localize("DSL.saves.magic.short"),
        icon: '<i class="fas fa-magic"></i>',
        callback: (html) => {
          rolled = true;
          rollData.form = html[0].querySelector("form");
          rollData.parts.push(`${rollData.data.roll.magic}`);
          rollData.title += ` ${game.i18n.localize("DSL.saves.magic.short")} (${
      ...
      cancel: {
        ...
        label: game.i18n.localize("DSL.Cancel"),
        callback: (html) => {},
      },
    };
  ...
  async Roll({
    ...
    if (skipDialog) {
      return ["melee", "missile", "attack"].includes(data.roll.type)
        ? DslDice.sendAttackRoll(rollData)
        : DslDice.sendRoll(rollData);
    }
