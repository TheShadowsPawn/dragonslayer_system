import { createDslMacro } from '../../module/macros';
import { trashChat, waitForInput } from '../testUtils';


export const key = 'dsl.actor.macro';
...
export default ({ before, beforeEach, after, describe, it, expect, assert, ...context }) => {
    ...
    const canCreate = async (type) => {
        ...
        const macro = await createDslMacro(data, 1);
        await waitForInput();

        const createdMacro = game.user.getHotbarMacros()[0];
        expect(createdMacro?.macro?.command.indexOf("game.dsl.rollItemMacro")).not.equal(-1)
    }
