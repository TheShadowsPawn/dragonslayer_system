export class DSLCombatant extends Combatant {
  ...
  getInitiativeRoll(formula: string) {
    ...
    if (this.isSlow) term = `${DSLCombatant.INITIATIVE_VALUE_SLOWED}`;
    if (this.isDefeated) term = `${DSLCombatant.INITIATIVE_VALUE_DEFEATED}`;
