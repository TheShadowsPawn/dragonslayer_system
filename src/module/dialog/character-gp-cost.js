...
export default class DslCharacterGpCost extends FormApplication {
  ...
  static get defaultOptions() {
    ...
    options.classes = ["dsl", "dialog", "gp-cost"];
    options.id = "sheet-gp-cost";
    options.template = `${DSL.systemPath()}/templates/actors/dialogs/gp-cost-dialog.html`;
    options.width = 240;
    return options;
  }
   ...
   * @property {boolean} preventCldsl - Should the application be stopped from closing?
  ...
  async _onSubmit(event, { preventCldsl = false, preventRender = false } = {}) {
    // eslint-disable-next-line no-underscore-dangle
    ...
    const gp = await this.object.items.find((item) => {
      ...
      return (
        (item.name === game.i18n.localize("DSL.items.gp.short") ||
          item.name === "GP") && // legacy behavior used GP, even for other languages
        itemData.treasure
      );
    });
    if (!gp) {
      ui.notifications.error(game.i18n.localize("DSL.error.noGP"));
      return;
    }
    ...
    if (newGP >= 0) {
      ...
      ui.notifications.error(game.i18n.localize("DSL.error.notEnoughGP"));
    }
  }
  ...
  async #updateObject(event, formData) {
    ...
    const content = await renderTemplate(
      `${DSL.systemPath()}/templates/chat/inventory-list.html`,
      templateData
    );
    ChatMessage.create({
      content,
