...
export default class DslCharacterCreator extends FormApplication {
  static get defaultOptions() {
    const options = super.defaultOptions;
    options.classes = ["dsl", "dialog", "creator"];
    options.id = "character-creator";
    options.template = `${DSL.systemPath()}/templates/actors/dialogs/character-creation.html`;
    options.width = 235;
    options.submitOnCldsl = false;
    return options;
  }
   ...
   */
  get title() {
    return `${this.object.name}: ${game.i18n.localize("DSL.dialog.generator")}`;
  }
  ...
  rollScore(score, options = {}) {
    ...
    const label =
      score === "gold"
        ? "Gold"
        : game.i18n.localize(`DSL.scores.${score}.long`);
    const rollParts = ["3d6"];
    const data = {
      roll: {},
    };
    ...
    return DslDice.Roll({
      ...
      flavor: game.i18n.format("DSL.dialog.generateScore", {
        score: label,
        count: this.counters[score],
      }),
      title: game.i18n.format("DSL.dialog.generateScore", {
        score: label,
        count: this.counters[score],
      }),
    });
  ...
  async _onSubmit(
    event,
    { updateData = null, preventCldsl = false, preventRender = false } = {}
  ) {
    ...
    const templateData = {
      config: CONFIG.DSL,
      scores: this.scores,
      title: game.i18n.localize("DSL.dialog.generator"),
      stats: this.object.stats,
      gold: this.gold,
    };
    const content = await renderTemplate(
      `${DSL.systemPath()}/templates/chat/roll-creation.html`,
      templateData
    );
    ...
    // Generate gold
    const itemData = {
      name: game.i18n.localize("DSL.items.gp.short"),
      type: "item",
      img: `${DSL.assetsPath}/gold.png`,
      system: {
        treasure: true,
        cost: 1,
        weight: 1,
