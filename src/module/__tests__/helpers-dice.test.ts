...
export const key = "dsl.helpers.dice";
export const options = {
  displayName: "DSL: Helpers: Dice",
};
...
export default ({
  ...
  describe("sendRoll(parts, data, title, flavor, speaker, form, chatMessage)", () => {
    ...
    it("Can roll with single part", async () => {
      const dice = await DslDice.sendRoll({
        parts: ["1d10"],
  ...
  describe("digestResult(data, roll)", () => {
    ...
    describe("result type", () => {
      ...
      it("Successful roll hitting target", () => {
        ...
        expect(DslDice.digestResult(data, roll).isSuccess).equal(true);
        expect(DslDice.digestResult(data, roll).isFailure).equal(false);
      });

      it("Unsuccessful roll under", () => {
        const roll = createMockRoll(9);
        expect(DslDice.digestResult(data, roll).isSuccess).equal(false);
        expect(DslDice.digestResult(data, roll).isFailure).equal(true);
      });
    ...
    describe("check type", () => {
      ...
      it("Unsuccessful roll on a nat 20", () => {
        ...
        expect(DslDice.digestResult(dataNat20, roll).isSuccess).equal(false);
        expect(DslDice.digestResult(dataNat20, roll).isFailure).equal(true);
      });
    ...
    describe("table type", () => {
      ...
      it("Successful roll hitting target", () => {
        ...
        expect(DslDice.digestResult(data, roll).isSuccess).equal(false);
        expect(DslDice.digestResult(data, roll).isSuccess).equal(false);
        expect(DslDice.digestResult(data, roll).details).equal("Success");
      });
      ...
      it("Successful roll under", () => {
        ...
        expect(DslDice.digestResult(data, roll).isSuccess).equal(false);
        expect(DslDice.digestResult(data, roll).isSuccess).equal(false);
        expect(DslDice.digestResult(data, roll).details).equal("Failure");
      });
    });
    ...
    describe("unknown type", () => {
      ...
      it("Returns default result", () => {
        ...
        expect(DslDice.digestResult(data, roll).isSuccess).equal(false);
        expect(DslDice.digestResult(data, roll).isSuccess).equal(false);
        expect(DslDice.digestResult(data, roll).target).equal(10);
        expect(DslDice.digestResult(data, roll).total).equal(4);
      });
    });
  });
  ...
  describe("attackIsSuccess(roll, target, bonus)", () => {
    describe("Ascending AC", () => {
      ...
      it("Natural 1 always fails", () => {
        ...
        expect(DslDice.attackIsSuccess(rollTargetOne, 100, 0)).equal(false);
        const rollTargetTwenty = createMockRoll(20, [1]);
        expect(DslDice.attackIsSuccess(rollTargetTwenty, 100, 0)).equal(false);
      });

      it("Natural 20 always succeeds", () => {
        const rollTargetOne = createMockRoll(1, [20]);
        expect(DslDice.attackIsSuccess(rollTargetOne, 0, 100)).equal(true);
        const rollTargetTwenty = createMockRoll(20, [20]);
        expect(DslDice.attackIsSuccess(rollTargetTwenty, 0, 100)).equal(true);
      });
      ...
      it("Roll + bonus equal target is successful", () => {
        ...
        const target = 20;
        const bonus = 10;
        expect(DslDice.attackIsSuccess(roll, target, bonus)).equal(true);
      });
      ...
      it("Roll + bnous under target is unsuccessful", () => {
        ...
        const target = 21;
        const bonus = 10;
        expect(DslDice.attackIsSuccess(roll, target, bonus)).equal(false);
      });
    });
    ...
    describe("Descending AC", () => {
      ...
      it("Roll + ac equal thac0 is successful", () => {
        ...
        const thac0 = 20;
        const ac = 10;
        expect(DslDice.attackIsSuccess(roll, thac0, ac)).equal(true);
      });
      ...
      it("Roll + ac under thac0 is unsuccessful", () => {
        ...
        const thac0 = 21;
        const ac = 10;
        expect(DslDice.attackIsSuccess(roll, thac0, ac)).equal(false);
      });
    });
  });
  ...
  describe("digestAttackResult(data, roll)", () => {
    ...
    describe("Ascending AC", () => {
      ...
      it("Natural 1 terms is unsuccessful", async () => {
        ...
        const rollTargetOne = createMockRoll(1, [1]);
        expect(DslDice.digestAttackResult(data, rollTargetOne).isSuccess).equal(
          false
        );
        expect(DslDice.digestAttackResult(data, rollTargetOne).isFailure).equal(
          true
        );
        const rollTargetTwenty = createMockRoll(20, [1]);
        expect(
          DslDice.digestAttackResult(data, rollTargetTwenty).isSuccess
        ).equal(false);
        expect(
          DslDice.digestAttackResult(data, rollTargetTwenty).isFailure
        ).equal(true);
      });
      ...
      it("Attack rolls with a modified result of 1 are allowed to succeeed if hits target AC. Issue#340", () => {
        ...
        expect(DslDice.digestAttackResult(targetData, roll).isSuccess).equal(
          true
        );
        expect(DslDice.digestAttackResult(targetData, roll).isFailure).equal(
          false
        );
      });
      ...
      it("Lower than target AC is unsuccessful", () => {
        ...
        expect(DslDice.digestAttackResult(data, roll).isSuccess).equal(false);
        expect(DslDice.digestAttackResult(data, roll).isFailure).equal(true);
      });
      ...
      it("Equal than target AC is successful", () => {
        ...
        expect(DslDice.digestAttackResult(data, roll).isSuccess).equal(true);
        expect(DslDice.digestAttackResult(data, roll).isFailure).equal(false);
      });
  ...
  describe("sendAttackRoll(parts, data, flags, title, flavor, speaker, form)", () => {
    ...
    it("Missing dmg roll shows notification", async () => {
      ...
      await DslDice.sendAttackRoll(rollData);
      await waitForInput();
