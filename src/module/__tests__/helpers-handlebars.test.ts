...
export const key = "dsl.helpers.handlebars";
export const options = {
  displayName: "DSL: Helpers: Handlebars",
};
  ...
  describe("registerHelpers()", () => {
    ...
    describe("getTagIcon helper", () => {
      ...
      describe("is functional", () => {
        const tags = Object.keys(CONFIG.DSL.tags);
        tags.forEach((tag) => {
          it(`for ${tag} tag`, async () => {
            expect(Handlebars.helpers.getTagIcon(CONFIG.DSL.tags[tag])).equal(
              CONFIG.DSL.tag_images[tag]
            );
          });
        });
      });
