...
import { createDslMacro, rollItemMacro } from "../helpers-macros";

export const key = "dsl.helpers.macro";
export const options = { displayName: "DSL: Helpers: Macro" };
...
export default ({
  ...
  describe("createDslMacro(data, slot)", () => {
    ...
    it("Can drag Macro to hotbar", async () => {
      ...
      await createDslMacro(data, macroSlot);
    ...
    it("Dragging Actor to hotbar send notification", async () => {
      ...
      expect(ui.notifications?.queue.pop()?.message).equal(
        game.i18n.localize("DSL.warn.macrosNotAnItem")
      );
      expect(ui.notifications?.queue.length).equal(0);
      await actor?.delete();
    });
    ...
    it("Dragging World Item to hotbar send notification", async () => {
      ...
      expect(ui.notifications?.queue.length).equal(1);
      expect(ui.notifications?.queue.pop()?.message).equal(
        game.i18n.localize("DSL.warn.macrosOnlyForOwnedItems")
      );
      expect(ui.notifications?.queue.length).equal(0);
      await actor?.delete();
      ...
      itemTypes.forEach((type) => {
        it(`Dragging Actor ${type.capitalize()} to hotbar craetes macro`, async () => {
          ...
          expect(macro?.macro?.command).equal(
            `game.dsl.rollItemMacro("New Actor Test ${type.capitalize()}");`
          );
          await actor?.delete();
        });
      });
  ...
  describe("rollItemMacro(itemName)", () => {
    it("No scene creates warning", async () => {
      ...
      expect(ui.notifications?.queue.length).equal(1);
      expect(ui.notifications?.queue.pop()?.message).equal(
        game.i18n.localize("DSL.warn.macrosNoTokenOwnedInScene")
      );
      expect(ui.notifications?.queue.length).equal(0);
      await actor?.delete();
    });
    ...
    it("Duplicate item creates warning but also craetes rolls", async () => {
      ...
      expect(ui.notifications?.queue.length).equal(1);
      expect(ui.notifications?.queue.pop()?.message).equal(
        game.i18n.format("DSL.warn.moreThanOneItemWithName", {
    ...
    it("Missing item creates warning", async () => {
      ...
      expect(error?.message).equal(
        game.i18n.format("DSL.error.noItemWithName", {
          actorName: actor?.name,
          itemName: `New Actor Test ${type.capitalize()}`,
        })
      );
