...
import DslDataModelAbility from "../data-model-ability";

export const key = "dsl.item.datamodel.ability";
export const options = { displayName: "DSL: Item: Data Model: Ability" };

export default ({ describe, it, expect }: QuenchMethods) => {
  describe("manualTags", () => {
    const ability = new DslDataModelAbility();
  ...
  describe("autoTags", () => {
    ...
    it("Save create autoTags", () => {
      ...
      expect(ability.autoTags[2].label).equal(
        game.i18n.localize("DSL.saves.death.long")
      );
    ...
    it("Roll create autoTags", () => {
      ...
      expect(ability.autoTags[2].label).equal(
        `${game.i18n.localize("DSL.items.Roll")} 1d20+1 undefinednull`
      );
    });
  });
};
