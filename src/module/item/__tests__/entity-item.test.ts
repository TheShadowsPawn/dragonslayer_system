...
import DslItem from "../entity";

export const key = "dsl.item.entity";
export const options = { displayName: "DSL: Item: Entity" };
...
export default ({
  ...
  describe("create()", () => {
    ...
    it("Can create weapon DslItem", async () => {
      await testItemCreate("weapon");
    });
    it("Can create armor DslItem", async () => {
      await testItemCreate("armor");
    });
    it("Can create item DslItem", async () => {
      await testItemCreate("item");
    });
    it("Can create spell DslItem", async () => {
      await testItemCreate("spell");
    });
    it("Can create ability DslItem", async () => {
      await testItemCreate("ability");
    });
    it("Can create container DslItem", async () => {
    ...
    it('Can\'t create without name', async () => {
      await expect(DslItem.create({type: 'container'})).to.be.rejectedWith(
        Error, 
        "[DslItem.name] may not be a blank string")
    })
    it('Can\'t create without type', async () => {
      await expect(DslItem.create({name: 'Test Item'})).to.be.rejectedWith(
        Error, 
        "[DslItem.type] may not be a blank string")
    })
    it('Can\'t create without acceptable type', async () => {
      await expect(DslItem.create({name: 'Test Item', type: 'TEST'})).to.be.rejectedWith(
        Error, 
        "[DslItem.type] TEST is not a valid choice")
    }) 
    */
  });
  ...
  describe("prepareDerivedData()", () => {
    it("has the expected fields", async () => {
      const item = (await createWorldTestItem(type)) as DslItem;
  ...
  describe("rollWeapon(options)", () => {
    it("Actor with melee & missile weapon renders dialog", async () => {
      const actor: DslActor = await createMockActor();
      await createActorTestItem(actor, "weapon");
      const weapon: DslItem = actor?.items.contents[0];
  ...
  describe("rollFormula(options)", () => {
    it("Missing item.system.roll throws error", async () => {
      const item: DslItem = await createWorldTestItem("item");
      try {
    ...
    it("Casting a spell trigger a dialog", async () => {
      const item: DslItem = await createWorldTestItem("spell");
    ...
    it("A DslDice.Roll is returned from method", async () => {
      const item: DslItem = await createWorldTestItem("spell");
      await item.update({ system: { roll: "1d20+12" } });
  ...
  describe("spendSpell()", () => {
    it("Calling using non-spell item throws an error", async () => {
      const item: DslItem = await createWorldTestItem("weapon");
      try {
  ...
  describe("pushManualTag(values)", () => {
    ...
    // eslint-disable-next-line unicorn/consistent-function-scoping
    const pushManualTag = async (item: DslItem, tag: string) => {
      expect(item?.system?.tags.length).equal(0);
      await item?.pushManualTag([tag]);
    };
    ...
    const booleanTags = new Set([
      CONFIG.DSL.tags.melee,
      CONFIG.DSL.tags.slow,
      CONFIG.DSL.tags.missile,
    ]);
    ...
    booleanTags.forEach((t) => {
      it(`"${t}" tag activates the boolean but not a tag`, async () => {
        ...
        testSystemBooleans(item, {
          melee: true,
          slow: t === CONFIG.DSL.tags.slow,
          missile: t === CONFIG.DSL.tags.missile,
        });
        item.delete();
      });
  ...
  describe("getAutoTagList()", () => {
    it("Container does nothing", async () => {
      const item: DslItem = await createWorldTestItem("container");
    ...
    it("Missile weapon has ranges applied", async () => {
      const item: DslItem = await createWorldTestItem("weapon");
      await item.pushManualTag([CONFIG.DSL.tags.missile]);
    ...
    it("Default Armor has armor type applied", async () => {
      const item: DslItem = await createWorldTestItem("armor");
      const tagList = item.getAutoTagList();
      expect(tagList.length).equal(1);
    ...
    it("Abilities have requirements applied", async () => {
      const item: DslItem = await createWorldTestItem("ability");
