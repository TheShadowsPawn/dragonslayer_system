 ...
 */
export async function createDslMacro(data, slot) {
  if (data.type === "Macro") {
    return game.user.assignHotbarMacro(await fromUuid(data.uuid), slot);
  }
  if (data.type === "RollTable") {
    const command = `game.dsl.rollTableMacro("${data.uuid}");`;
  ...
  const command = `game.dsl.rollItemMacro("${item.name}");`;
...
export function rollTableMacro(tableUuId) {
  fromUuid(tableUuId).then((table) => {
    if (table === null) {
      return ui.notifications.error(
        game.i18n.format("DSL.error.noRollTableWithUuId", {
          uuid: tableUuId,
        })
      );
    }
