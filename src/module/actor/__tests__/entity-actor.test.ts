...
export const key = "dsl.actor.entity";
export const options = {
  displayName: "DSL: Actor: Entity",
  preSelected: true,
};
...
export default ({
  ...
  describe("createEmbeddedDocuments(embeddedName, data, context)", () => {
    ...
    itemTypes.forEach((itemType) => {
      it(`Creates ${itemType.capitalize()} with correct default icon`, async () => {
        const actor = await createMockActor("character");
        const item = (await DslItem.create({
          name: `Test ${key} ${itemType.capitalize()}`,
          type: itemType,
        })) as DslItem;
        await actor?.createEmbeddedDocuments("Item", [item]);
        const actorItem = actor?.items.getName(item?.name);
        expect(actorItem?.img).equal(DslItem.defaultIcons[itemType]);
        item?.delete();
      });
    });
  });
  ...
  describe("isNew()", () => {
    ...
    it("monster", async () => {
      const actor = (await createMockActor("monster")) as DslActor;
      expect(actor?.isNew()).equal(true);
      await actor?.delete();
    });
  });
    ...
    describe("for monster", () => {
      it("in wilderness", async () => {
        ...
        expect(game.messages?.contents[0].content).contain(
          game.i18n.format("DSL.roll.appearing", { type: "(2)" })
        );
        await actor.delete();
      });
      ...
      it("in other", async () => {
        ...
        expect(game.messages?.size).equal(1);
        expect(game.messages?.contents[0].content).contain(
          game.i18n.format("DSL.roll.appearing", { type: "(1)" })
        );
        await actor.delete();
      });
    });
  ...
  describe("rollExploration(expl, options)", () => {
    ...
    explorationOptions.forEach((expl) => {
      it("for character", async () => {
        ...
        await waitForInput();
        expect(game.messages?.contents[0].content).contain(
          game.i18n.format("DSL.roll.exploration", {
            exploration: game.i18n.localize(`DSL.exploration.${expl}.long`),
          })
        );
        await actor.delete();
      });
  ...
  describe("rollDamage(attData, options)", () => {
    ...
    it("for character", async () => {
      ...
      expect(game.messages?.size).equal(1);
      expect(game.messages?.contents[0].content).contain(
        `test - ${game.i18n.localize("DSL.Damage")}`
      );
      await actor.delete();
    });
