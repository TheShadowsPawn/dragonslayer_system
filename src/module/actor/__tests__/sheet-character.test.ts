...
import DslActorSheetCharacter from "../character-sheet";

export const key = "dsl.actor.sheet.character";
export const options = { displayName: "DSL: Actor: Sheet: Character" };
...
export default ({ describe, it, expect, after, afterEach }: QuenchMethods) => {
  ...
  describe("defaultOptions()", () => {
    it("Has correctly set defaultOptions", async () => {
      ...
      const sheet = actor?.sheet as unknown as DslActorSheetCharacter;
  ...
  describe("_chodslLang()", () => {
    it("renders a dialog", async () => {
      const actor = await createMockActorKey("character", {}, key);
      // eslint-disable-next-line no-underscore-dangle
      actor?.sheet?._chodslLang();
      await waitForInput();
