...
import DslActor from "../entity";

export const key = "dsl.actor.sheet";
export const options = {
  displayName: "DSL: Actor: Sheet",
  preSelected: true,
};
...
export default ({
  ...
  describe("getData()", () => {
    it("returns the expected data", async () => {
      ...
      const sheet = new DslActorSheet(actor);
    ...
    itemTypes.forEach((itemType) => {
      describe(`${itemType} item`, () => {
        it("can remove item outside container", async () => {
          const actor = (await getActor()) as DslActor;
    ...
    describe(`character can roll`, () => {
      ...
      saves.forEach((save) => {
        it(`${save} save`, async () => {
          ...
          expect(game.messages?.size).equal(1);
          expect(game.messages?.contents[0].content).contain(
            game.i18n.format("DSL.roll.save", {
              save: game.i18n.localize(`DSL.saves.${save}.long`),
            })
          );
        });
      });
  ...
  describe("_rollAttack(event)", () => {
    ...
    attackTypeClasses.forEach((attackClass) => {
      it(`can attack with ${attackClass}`, async () => {
        ...
        expect(game.messages?.size).equal(1);
        expect(game.messages?.contents[0].content).contain(
          game.i18n.format("DSL.roll.attacks", { name: actor?.name })
        );
        await trashChat();
      });
    });
  ...
  describe("_onDropItemCreate(droppedItem, targetContainer)", () => {
    ...
    itemTypes.forEach((itemType) => {
      ...
      it(`add non-actor ${itemType} to sheet`, async () => {
        ...
        items.source.item = (await createWorldTestItem(itemType)) as DslItem;
  ...
  describe("_chodslItemType(choices)", () => {
    ...
    it("can create custom dialog", async () => {
      ...
      const actor = (await createMockActorKey("monster", {}, key)) as DslActor;
      // eslint-disable-next-line no-underscore-dangle
      actor?.sheet?._chodslItemType(customChoices);
      await waitForInput();
