...
import DslEntityTweaks from "../dialog/entity-tweaks";

export default class DslActorSheet extends ActorSheet {
  getData() {
  ...
  async _onDropItem(event, data) {
    ...
    // Issue: https://github.com/vttred/dsl/issues/357
  ...
  async _chodslItemType(choices = ["weapon", "armor", "shield", "gear"]) {
    const templateData = { types: choices };
    const dlg = await renderTemplate(
      `${DSL.systemPath()}/templates/items/entity-create.html`,
      templateData
    );
    // Create Dialog window
    return new Promise((resolve) => {
      new Dialog({
        title: game.i18n.localize("DSL.dialog.createItem"),
        content: dlg,
        buttons: {
          ok: {
  ...
  _createItem(event) {
    ...
    if (type === "choice") {
      ...
      this._chodslItemType(choices).then((dialogInput) => {
  ...
  _onConfigureActor(event) {
    event.preventDefault();
    new DslEntityTweaks(this.actor, {
  ...
  _getHeaderButtons() {
    ...
    if (this.options.editable && canConfigure) {
      buttons = [
        {
          label: game.i18n.localize("DSL.dialog.tweaks"),
  ...
  activateListeners(html) {
    ...
    html.find(".item-delete").click((event) => {
      ...
      Dialog.confirm({
        title: game.i18n.localize("DSL.dialog.deleteContainer"),
        content: game.i18n.localize("DSL.dialog.confirmDeleteContainer"),
        yes: () => {
