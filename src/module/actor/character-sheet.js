...
import DslCharacterCreator from "../dialog/character-creation";
import DslCharacterGpCost from "../dialog/character-gp-cost";
import DslCharacterModifiers from "../dialog/character-modifiers";
import DslActorSheet from "./actor-sheet";

export default class DslActorSheetCharacter extends DslActorSheet {
  /**
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["dsl", "sheet", "actor", "character"],
      template: `${DSL.systemPath()}/templates/actors/character-sheet.html`,
      width: 450,
      height: 530,
      resizable: true,
      tabs: [
  ...
  generateScores() {
    new DslCharacterCreator(this.actor, {
  ...
  async _chodslLang() {
    const choices = CONFIG.DSL.languages;

    const templateData = { choices };
    const dlg = await renderTemplate(
      `${DSL.systemPath()}/templates/actors/dialogs/lang-create.html`,
      templateData
    );
    ...
    return new Promise((resolve) => {
      new Dialog({
        ...
        buttons: {
          ok: {
            label: game.i18n.localize("DSL.Ok"),
  ...
  _pushLang(table) {
    ...
    this._chodslLang().then((dialogInput) => {
      const name = CONFIG.DSL.languages[dialogInput.choice];
      if (update.value) {
  ...
  _onShowModifiers(event) {
    ...
    new DslCharacterModifiers(this.actor, {
      top: this.position.top + 40,
      left: this.position.left + (this.position.width - 400) / 2,
    }).render(true);
  }
  ...
  async _onShowGpCost(event, preparedData) {
    event.preventDefault();
    new DslCharacterGpCost(this.actor, preparedData, {
      top: this.position.top + 40,
      left: this.position.left + (this.position.width - 400) / 2,
    }).render(true);
  }
  ...
  async _onShowItemTooltip(event) {
    const templateData = {};
    const dlg = await renderTemplate(
      `${DSL.systemPath()}/templates/actors/partials/character-item-tooltip.html`,
      templateData
    );
