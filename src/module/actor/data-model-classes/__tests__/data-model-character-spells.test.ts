...
import DslDataModelCharacterSpells from "../data-model-character-spells";

export const key = "dsl.actor.datamodel.character.spells";
export const options = {
  displayName: "DSL: Actor: Data Model: Character Spells",
};
...
export default ({ describe, it, expect }: QuenchMethods) => {
  ...
  describe("Spell levels", () => {
    it("Sorts the incoming spell list into an object with spell level keys", () => {
      ...
      const spellData = new DslDataModelCharacterSpells({}, spells);
  ...
  describe("Spell slots", () => {
    describe("Shows committed and max spell slots per level", () => {
      it("with no spells prepared", () => {
        ...
        const spellData = new DslDataModelCharacterSpells(
          { 1: { max: 1 } },
          spells as Item[]
        );
  ...
  describe("Checking for spellcasting", () => {
    it("Can cast spells when spellcasting is enabled", () => {
      const spellData = new DslDataModelCharacterSpells({ enabled: true }, []);
      expect(spellData.enabled).to.be.true;
    });
    it("Cannot cast spells when spellcasting is disabled", () => {
      const spellData = new DslDataModelCharacterSpells({ enabled: false }, []);
