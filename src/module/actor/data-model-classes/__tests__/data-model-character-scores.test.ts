...
import DslDataModelCharacterScores from "../data-model-character-scores";

export const key = "dsl.actor.datamodel.character.scores";
export const options = {
  displayName: "DSL: Actor: Data Model: Character Ability Scores",
};
...
export default ({ describe, it, expect }: QuenchMethods) => {
  ...
  const tables = [
    DslDataModelCharacterScores.standardAttributeMods,
    DslDataModelCharacterScores.cappedAttributeMods,
    DslDataModelCharacterScores.openDoorMods,
    DslDataModelCharacterScores.literacyMods,
    DslDataModelCharacterScores.spokenMods,
  ];
  const fromTable = (tableKey: number, score: number) =>
    DslDataModelCharacterScores.valueFromTable(tables[tableKey], score);
  ...
  const buildTestCases = (
    ...
    const scoresObj = new DslDataModelCharacterScores(scoresToUse);
    return it(`${score}`, () => {
