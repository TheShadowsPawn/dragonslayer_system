import DslItem from "../item/entity";

import skipRollDialogCheck from "../helpers-behaviour";
import DslDice from "../helpers-dice";
...
export default class DslActor extends Actor {
  prepareDerivedData() {
  ...
  async createEmbeddedDocuments(embeddedName, data = [], context = {}) {
    data.map((item) => {
      if (item.img === undefined) {
        item.img = DslItem.defaultIcons[item.type];
      }
    });
  ...
  getExperience(value, options = {}) {
    ...
    return this.update({
      ...
      ChatMessage.create({
        content: game.i18n.format("DSL.messages.GetExperience", {
          name: this.name,
          value: modified,
        }),
        speaker,
  ...
  generateSave(hd) {
    ...
    let saves = {};
    for (let i = 0; i <= hd; i++) {
      const tmp = CONFIG.DSL.monster_saves[i];
      if (tmp) {
        saves = tmp;
      }
    }
    ...
    // Compute Thac0
    let thac0 = 20;
    Object.keys(CONFIG.DSL.monster_thac0).forEach((k) => {
      if (hd < parseInt(k)) return;
      thac0 = CONFIG.DSL.monster_thac0[k];
    });
  ...
  rollSave(save, options = {}) {
    const label = game.i18n.localize(`DSL.saves.${save}.long`);
    ...
    const data = {
      ...
      details: game.i18n.format("DSL.roll.details.save", { save: label }),
    };

    const rollMethod =
      actorType === "character" ? DslDice.RollSave : DslDice.Roll;
    ...
    return rollMethod({
      ...
      flavor: game.i18n.format("DSL.roll.save", { save: label }),
      title: game.i18n.format("DSL.roll.save", { save: label }),
  ...
  rollMorale(options = {}) {
    ...
    return DslDice.Roll({
      ...
      skipDialog: true,
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: game.i18n.localize("DSL.roll.morale"),
      title: game.i18n.localize("DSL.roll.morale"),
    });
  }

  rollLoyalty(options = {}) {
    const label = game.i18n.localize(`DSL.roll.loyalty`);
    const rollParts = ["2d6"];
  ...
  rollReaction(options = {}) {
    ...
    const data = {
      ...
      roll: {
        type: "table",
        table: {
          2: game.i18n.format("DSL.reaction.Hostile", {
            name: this.name,
          }),
          3: game.i18n.format("DSL.reaction.Unfriendly", {
            name: this.name,
          }),
          6: game.i18n.format("DSL.reaction.Neutral", {
            name: this.name,
          }),
          9: game.i18n.format("DSL.reaction.Indifferent", {
            name: this.name,
          }),
          12: game.i18n.format("DSL.reaction.Friendly", {
            name: this.name,
          }),
        },
      },
    ...
    return DslDice.Roll({
      ...
      skipDialog: options.fastForward || skipRollDialogCheck(options.event),
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: game.i18n.localize("DSL.reaction.check"),
      title: game.i18n.localize("DSL.reaction.check"),
    });
  }
  ...
  rollCheck(score, options = {}) {
    ...
    const label = game.i18n.localize(`DSL.scores.${score}.long`);
    const rollParts = ["1d20"];
    ...
    const data = {
      ...
      details: game.i18n.format("DSL.roll.details.attribute", {
        score: label,
      }),
    };
    ...
    return DslDice.Roll({
      ...
      skipDialog: options.fastForward || skipRollDialogCheck(options.event),
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: game.i18n.format("DSL.roll.attribute", { attribute: label }),
      title: game.i18n.format("DSL.roll.attribute", { attribute: label }),
      chatMessage: options.chatMessage,
    });
  }
  ...
  rollHitDice(options = {}) {
    ...
    const label = game.i18n.localize(`DSL.roll.hd`);
  ...
  rollAppearing(options = {}) {
    ...
    return DslDice.Roll({
      ...
      skipDialog: true,
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: game.i18n.format("DSL.roll.appearing", { type: label }),
      title: game.i18n.format("DSL.roll.appearing", { type: label }),
    });
  }
  ...
  rollExploration(expl, options = {}) {
    ...
    const label = game.i18n.localize(`DSL.exploration.${expl}.long`);
    const rollParts = ["1d6"];
    ...
    const data = {
      ...
      details: game.i18n.format("DSL.roll.details.exploration", {
        expl: label,
      }),
    };
    ...
    return DslDice.Roll({
      ...
      skipDialog: options.fastForward || skipRollDialogCheck(options.event),
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: game.i18n.format("DSL.roll.exploration", { exploration: label }),
      title: game.i18n.format("DSL.roll.exploration", { exploration: label }),
    });
  }
  ...
  rollDamage(attData, options = {}) {
    ...
    DslDice.Roll({
      ...
      skipDialog: true,
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: `${attData.label} - ${game.i18n.localize("DSL.Damage")}`,
      title: `${attData.label} - ${game.i18n.localize("DSL.Damage")}`,
    });
  }
   ...
   * @param {DslItem} attData.item - the item being used in the attack
   * @param {DslActor} attData.actor - this actor (at least it should be!)
   ...
   * @param {DslActor} attData.roll.target - the target of the attack
   ...
   * @returns {DslDice.Roll} - the attack roll with completed rollParts and other data
   */
  rollAttack(attData, options = {}) {
    const data = this.system;

    const label = attData.item
      ? game.i18n.format("DSL.roll.attacksWith", {
          name: attData.item.name,
        })
      : game.i18n.format("DSL.roll.attacks", {
          name: this.name,
        });
    ...
    return DslDice.Roll({
      ...
      speaker: ChatMessage.getSpeaker({ actor: this }),
      flavor: label,
      flags: { dsl: { roll: "attack", itemId: attData.item?._id } },
      title: label,
    });
  }
