 ...
 */
export default class DslActorSheetMonster extends DslActorSheet {
  /**
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["dsl", "sheet", "monster", "actor"],
      template: `${DSL.systemPath()}/templates/actors/monster-sheet.html`,
      width: 450,
      height: 560,
      resizable: true,
      tabs: [
   ...
   */
  async generateSave() {
    const choices = CONFIG.DSL.monster_saves;

    const templateData = { choices };
    const dlg = await renderTemplate(
      `${DSL.systemPath()}/templates/actors/dialogs/monster-saves.html`,
      templateData
    );
    // Create Dialog window
    new Dialog(
      {
        title: game.i18n.localize("DSL.dialog.generateSaves"),
        content: dlg,
        buttons: {
          ok: {
  ...
  _cycleAttackPatterns(event) {
    ...
    // Attack patterns include all DSL colors and transparent
